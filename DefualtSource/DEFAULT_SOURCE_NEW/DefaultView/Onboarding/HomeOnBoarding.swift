    ...
    var body: some View {
        GeometryReader{
            ...
            IntroView(intro: $activeIntro, size: size){
                ...
                VStack(spacing: 10){
                    //custom text field
                    CustomTextField(text: $authManagerViewModel.email, hint: "Email Address", leadignIcon: Image(systemName: "envelope"))
                    CustomTextField(text: $authManagerViewModel.pswd, hint: "Password", leadignIcon: Image(systemName: "lock"), isPassword: true)
                    HStack{
                        styledText(text: "Not now ?", foregroundColor: .text, font: .regular(size: 15))
                            .onTapGesture {
                                withAnimation {
                                    appController.firstOpen = false
                                }
                            }
                            .padding(.top,5)
                    ...
                    Button{
                        if authManagerViewModel.email.isEmpty && authManagerViewModel.pswd.isEmpty{
                        ...
                        Text("\(authManagerViewModel.isSignin ? "Continue" : "Sign up")")
                            .fontWeight(.semibold)
...
struct HomeOnBoarding_Previews: PreviewProvider {
